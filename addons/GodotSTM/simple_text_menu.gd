...
export(bool) var menu_enabled = true setget set_menu
...
func _input(event):
	if (event.type == InputEvent.KEY and event.pressed):
		if event.scancode == KEY_UP:
			if(orientation == 1):
				set_cursor_option(_current_option + _options.get_child_count() - 1)
		...
		if event.scancode == KEY_Z:
			if(signal_argument == 0):
				...
				emit_signal("option_selected", _options.get_child(_current_option).get_text())
		_cursor_timer = 0.0
