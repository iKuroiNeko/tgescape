...
func _ready():
	...
	if global.get_playerName() == null:
		Globals.set("playerName","Haku")
...
#			audio.play("beep_letter")
		...
		if position==0:
			...
			self.set_pos(Vector2(0,screen_res.size.y-(get_size().y + 5)))
		var size = get_size().x
...
#	if position <= 1:
	btn_answers.set_pos(Vector2(textObj.get_pos().x, get_rect().size.y/2 - 35))
#	else:
...
#		index+=1
	...
	btn_answers.update()
	yield(get_tree(), "idle_frame")
	btn_answers.set_size(Vector2(get_size().x/2,btn_answers.get_size().y))
	btn_answers.update()
	btn_answers.set_margin(MARGIN_RIGHT, 0)
	btn_answers.set_opacity(1)
