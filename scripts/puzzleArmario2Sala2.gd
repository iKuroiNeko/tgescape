...
func _ready():
	...
	resolvido = global.get_puzzleArmario2Sala2Res()
...
func _on_TIE_input_enter( input ):
	...
	if input == "3798":
		resolvido = true
		global.set_puzzleArmario2Sala2Res(resolvido)
		tie.buff_text("O armário se abriu! Você encontrou um pano sujo.", 0.005)
		inventory.addItem("Pedaço de pano sujo")
		global.set_panoSujoPicked(true)
		buffEnd = true
	else:
		tie.buff_text(".....", 0.005)
		buffEnd = true
